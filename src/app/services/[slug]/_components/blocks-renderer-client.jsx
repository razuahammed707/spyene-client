...
const ImageBlock = ({ image, placeholder_url }) => {
  const [imgSrc, setImgSrc] = React.useState(image.url || "/service/card_placeholder.png");
  return (
    <Image
      src={imgSrc}
      onError={() => setImgSrc("/service/card_placeholder.png")}
...
const BlockRendererClient = ({ content }) => {
  ...
  return (
    <BlocksRenderer
      content={content}
      blocks={{
        image: ({ image }) => <ImageBlock image={image} placeholder_url={placeholder_url} />,
        paragraph: ({ children }) => (
